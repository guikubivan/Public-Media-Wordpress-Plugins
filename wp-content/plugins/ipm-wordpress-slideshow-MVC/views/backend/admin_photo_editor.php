	...
	<? if ($slideshow_id != "single") {?> <div onclick='setCoverImage("<?php echo $slideshow_id ?>", "<?php echo $photo->photo_id?>");' id='cover_button_<?php echo $slideshow_id ?>_<?php echo $photo->photo_id?>' class='button set_cover_button_<?php echo $slideshow_id ?>' style='text-align:center;margin-top:5px;' ><? if(isset($ss_thumb) && !empty($ss_thumb) && $ss_thumb == $photo->photo_id) {?>This is the slideshow thumbnail<?}else{?>Make this the slideshow thumbnail<?}?></div> <? } ?>
				...
				<div>
					<label >Title:</label>
					<input type='text'  id='slideshowItem_<?php echo $slideshow_id ?>_photos_<?php echo $photo->photo_id ?>_title' name='slideshow[<?php echo $slideshow_id ?>][photos][<?php echo $photo->photo_id ?>][title]' style='width:250px' value="<?php echo htmlentities($photo->title, NULL, 'UTF-8')?>" class='<?php echo $this->plugin_prefix ?>required photo_title' />
				</div>
				<div>	
					<label >Photo credit</label>
					<input type='text' id='slideshowItem_<?php echo $slideshow_id ?>_photos_<?php echo$photo->photo_id?>_photo_credit' name='slideshow[<?php echo $slideshow_id ?>][photos][<?php echo $photo->photo_id ?>][photo_credit]' style='width:250px' value="<?php echo htmlentities($photo->photo_credit, NULL, 'UTF-8') ?>"  title='Click to edit' class='editable <?php echo $this->plugin_prefix ?>required' />
				</div>
				<div>
				...
				<div>
					...
					<input type='text' id='slideshowItem_<?php echo $slideshow_id ?>_photos_<?php echo$photo->photo_id?>_original_url' name='slideshow[<?php echo $slideshow_id ?>][photos][<?php echo $photo->photo_id ?>][original_url]' style='width:250px' value="<?php echo htmlentities($photo->original_url, NULL, 'UTF-8') ?>"  title='Click to edit' class='editable' />
				</div>
				<div>
					<label >Alt text <small>Specific to this Slideshow</small></label>
					<input type='text' id='slideshowItem_<?php echo $slideshow_id ?>_photos_<?php echo $photo->photo_id?>_alt' name='slideshow[<?php echo $slideshow_id ?>][photos][<?php echo $photo->photo_id ?>][alt]' style='width:250px' value="<?php echo htmlentities($photo->alt, NULL, 'UTF-8') ?>" class='editable <?php echo $this->plugin_prefix ?>required' />
					
				</div>
				
				<div>
					<label >Caption</label>
					<textarea  id='slideshowItem_<?php echo $slideshow_id ?>_photos_<?php echo $photo->photo_id?>_caption' name='slideshow[<?php echo $slideshow_id ?>][photos][<?php echo $photo->photo_id ?>][caption]' rows='2' style='width:250px' class='<?php echo $this->plugin_prefix ?>required' ><?php echo htmlentities($photo->caption, NULL, 'UTF-8') ?></textarea>
				</div>
