					...
					$(select).next().bind("click", function(e){
						if($(select).val() > -1 ){
							...
							events.program_id[cur_id] = programsXML.getElementsByTagName("ID")[xmlIndex].childNodes[0].nodeValue;
                                                        //don't change the color of the container unless it's div
                                                        if(events.container[cur_id][0].tagName.toLowerCase() == 'div')
...
<? if($edit): ?>
				clearAllEvents();
				$(obj).find('#0_0').find("div:first").prepend('<div class="loading_div calendar_overlay" style="height: ' + (opts.cell_height*(opts.hour_fraction*24)) + 'px; width: ' + (opts.cell_width*7+7) + 'px;"><img src="' + WPScheduler.url + '/wp-content/plugins/program_scheduler/images/loading.gif" /></div>');
...
<? else: ?>
				...
				if( newDate.getDate() != start_datetime.getDate() ){
					clearAllEvents();
					$(obj[objIndex]).find('#0_0').find("div:first").prepend('<div class="loading_div calendar_overlay" style="height: ' + (opts.cell_height*(opts.hour_fraction*24)) + 'px; width: ' + (opts.cell_width*7-35) + 'px;"><img src="' + WPScheduler.url + '/wp-content/plugins/program_scheduler/images/loading.gif" /></div>');
...
<? endif; ?>
			...
			var programsXML;
<? if($edit): ?>	var updateXML = true; <? endif; ?>
			...
			var cur_id = -1;
<? if(!$edit): ?>	var schedule_name = isArray(opts.schedule_name) ? opts.schedule_name[objIndex] : opts.schedule_name;<? endif; ?>
			ajax_get_programs('load');
